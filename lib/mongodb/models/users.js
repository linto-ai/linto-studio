...
function generateValidityDate(value, periodType) {
  return moment().add(value, periodType).format()
}
...
class UsersModel extends MongoModel {
    ...
    async createUser(payload) {
        try {
            ...
            const validityDate = generateValidityDate(30, 'm') // 30 minutes
            const userPayload = {
              email: payload.email,
    ...
    async createExternalUser(payload) {
      const magicId = randomstring.generate({charset: 'alphanumeric', length: 20})
      const validityDate = generateValidityDate('2', d) // 2 days
      try {
          const userPayload = {
              email: payload.email,
