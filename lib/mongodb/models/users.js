...
function generateValidityDate(date) {
  return moment().add(date.time, date.period).format()
}
...
class UsersModel extends MongoModel {
    ...
    async createUser(payload) {
        try {
            ...
            const validityDate = generateValidityDate(VALIDITY_DATE.SHORT) // 30 minutes
            const userPayload = {
              email: payload.email,
    ...
    async createExternalUser(payload) {
      const magicId = randomstring.generate({charset: 'alphanumeric', length: 20})
      const validityDate = generateValidityDate(VALIDITY_DATE.LONG) // 2 days
      try {
          const userPayload = {
              email: payload.email,
    ...
    async setUserMagicLink(email) {
      try {
        ...
        const magicId = randomstring.generate({charset: 'alphanumeric', length: 20})
        const validityDate = generateValidityDate(VALIDITY_DATE.SHORT)
