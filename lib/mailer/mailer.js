...
class Mailer {
  ...
  static async sendMail(mail_payload) {
    try {
      ...
      if (mail_result.accepted.length > 0 && mail_result.accepted.indexOf(mail_payload.to) >= 0)
        return true
      else if (mail_result.rejected.length > 0 && mail_result.rejected.indexOf(mail_payload.to) >= 0)
        return false
      else throw new MailError('Mailer was not able to identify the mail result')
    } catch (err) {
      return false
    }
  }
  ...
  static checkConfig() {
    if (process.env.SMTP_HOST && process.env.SMTP_PORT && process.env.SMTP_SECURE &&
      process.env.SMTP_REQUIRE_TLS && process.env.SMTP_AUTH && process.env.SMTP_PSWD) {
      return true
    } else {
      return false
    }
