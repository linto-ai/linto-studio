...
function shareMemberOrga(payload) {
  ...
  <div style="margin: 0 auto 20px auto; padding:20px; background-color: #fff; max-width: 960px; min-width: 320px;"><p style="margin:0;"><a href="${payload.origin}/interface/inbox?organizationId=${payload.organizationId}" target="_blank">${payload.origin}/interface/inbox?organizationId=${payload.organizationId}}</a></p></div>
  `
}
...
function shareMultipleConversation(payload) {
  ...
  for (let conv_id of payload.conversationsList) {
    conv_link += `<li><p style="margin:0;"><a href="https://${payload.origin}/interface/conversations/${conv_id}" target="_blank">https://${payload.origin}/interface/conversations/${conv_id}</a></p></li>`
  }
  conv_link += '</ul>'
