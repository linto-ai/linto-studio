...
class Mailing {
    ...
    async organizationInvite(email, origin, sharedByEmail, orgaName, organizationId) {
        if (typeof origin === 'object') origin = this.exctractRequestOrigin(origin)

        return await this.sendMail(TYPE.INVITE_ORGANIZATION, { email, origin }, { origin, sharedByEmail, orgaName, organizationId })
    }
