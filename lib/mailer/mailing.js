...
class Mailing {
    ...
    async conversationShared(email, origin, sharedByName, sharedByEmail, conversationId) {
        if (typeof origin === 'object') origin = this.exctractRequestOrigin(origin)

        return await this.sendMail(TYPE.SHARE_CONVERSATION, { email, origin }, { origin, sharedByName, sharedByEmail, conversationId })
    }

    async conversationSharedExternal(email, origin, magicId, sharedByName, sharedByEmail, conversationId) {
        if (typeof origin === 'object') origin = this.exctractRequestOrigin(origin)

        return await this.sendMail(TYPE.SHARE_CONVERSATION_EXTERNAL, { email, origin }, { origin, magicId, sharedByName, sharedByEmail, conversationId })
    }
