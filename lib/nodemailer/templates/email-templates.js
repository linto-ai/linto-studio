...
function htmlShareConversationExternalTemplate(payload) {
  ...
  <body>
    <div id="mail-wrapper">
      ...
      <div style="margin: 0 auto 20px auto; padding:20px; background-color: #fff; max-width: 960px; min-width: 320px;"><p style="margin:0;"><a href="${payload.reqOrigin}/magiclink-auth/${payload.magicId}?conversationId=${payload.conversationId}" target="_blank">${payload.reqOrigin}/magiclink-auth/${payload.magicId}?conversationId=${payload.conversationId}</a></p></div>
      <!-- END Body -->
      ${MAIL_FOOTER}
    </div>
  </body>
