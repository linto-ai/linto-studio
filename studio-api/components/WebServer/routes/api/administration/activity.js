const debug = require("debug")(
  "linto:components:WebServer:routes:api:administration:activity",
)

const {
  getActivity,
  getKpiByRessource,
  getKpiBySession,
  refreshSessionKpi,
  getKpiSeries,
  exportKpiSessions,
} = require(
  `${process.cwd()}/components/WebServer/routecontrollers/administration/activity.js`,
)

module.exports = (webserver) => {
  return [
    {
      path: "/activity/",
      method: "get",
      requireAuth: true,
      requireSuperAdmin: true,
      controller: getActivity,
    },
    {
      path: "/activity/compute",
      method: "get",
      requireAuth: true,
      requireSuperAdmin: true,
      controller: getKpiByRessource,
    },
    {
      path: "/activity/compute/series",
      method: "get",
      requireAuth: true,
      requireSuperAdmin: true,
      controller: getKpiSeries,
    },
    {
      path: "/activity/session",
      method: "get",
      requireAuth: true,
      requireSuperAdmin: true,
      controller: refreshSessionKpi,
    },
    {
      path: "/activity/session/export",
      method: "get",
      requireAuth: true,
      requireSuperAdmin: true,
      controller: exportKpiSessions,
    },
    {
      path: "/activity/session/:sessionId",
      method: "get",
      requireAuth: true,
      requireSuperAdmin: true,
      controller: getKpiBySession,
    },
  ]
}
