      ...
      controller: async (req, res, next) => {
        try {
          ...
          if (!sessionId || !transcriberId) {
            throw new DeliveryUnsupportedMediaType(
              `sessionId or transcriberId empty`,
            )
          }
          ...
          await Model.Session.findByPk(sessionId, {
            ...
            .then((session) => {
              ...
              for (channel of session.channels) {
                if (
                  (channel.transcriber_id ?? channel.transcriberProfileId) ==
                  transcriberId
                ) {
              ...
              throw new DeliveryNotFound(
                `No channel found for transcriberId ${transcriberId}`,
              )
            })
            .then(([session, channel]) => {
              try {
                fileGeneratorMapping[type](session, channel, timezone).then(
                  (content) => {
                ...
                throw new DeliveryError(
                  `Error while generating ${type} file`,
                  err,
                )
              }
            })
