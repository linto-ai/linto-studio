...
module.exports = function (segments, segmentWordResize) {
  ...
  for (let segment of segments) {
    const segment_to_splice = Math.ceil(segment.words.length / segmentWordResize)
    ...
    if (segment_to_splice <= 0) segment_resized.push(segment) // No need to resize
      ...
      for (let j = 0; j < segment_to_splice; j++) {
        ...
        let new_raw_words = segment.raw_words.filter(word => word.start >= first_start_word && word.end <= last_end_word)

        if (j === segment_to_splice - 1) { // if last segment words added, add all the remaining raw words
          let last_missed_words = segment.raw_words.filter(word => word.start >= last_end_word)
