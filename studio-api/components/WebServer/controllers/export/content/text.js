const debug = require('debug')(`linto:conversation-manager:components:WebServer:controllers:export:content:text`)

const docx = require("docx")
const { AlignmentType, Paragraph, Table, TableCell, TableRow,
    TabStopType, TextRun, WidthType, TableLayoutType } = docx
...
function processTurnTable(paragraphs_content, conversation, data, query) {
    ...
    let TZ, offsetMinutes, startTime = -1
    ...
    for (const text of conversation.text) {
        let datetime
        if (startTime !== -1) {
            const startDatetime = addSeconds(parseISO(startTime), text.stime + offsetMinutes * 60)
            datetime = format(startDatetime, 'HH:mm', { locale: en })
        } else {
            if (text.stime === undefined || text.etime === undefined) {
                datetime = text.words[0].stime + ' - ' + text.words[text.words.length - 1].etime
    ...
    paragraphs_content.push(new Table({
        rows: rows,
        width: {
            size: 100,
