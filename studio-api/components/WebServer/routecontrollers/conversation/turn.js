const debug = require("debug")(
  `linto:components:WebServer:routecontrollers:conversation:turn`,
)
...
async function addTurn(req, res, next) {
  try {
    ...
    requireParam(req.params.turnId, TurnIdRequire) // Will add a turn after that turnId
...
async function deleteTurn(req, res, next) {
  try {
    requireParam(req.params.conversationId, ConversationIdRequire)
    requireParam(req.params.turnId, TurnIdRequire)
...
async function mergeTurn(req, res, next) {
  try {
    requireParam(req.params.conversationId, ConversationIdRequire)
    requireParam(req.params.turnId, TurnIdRequire)
    requireParam(req.params.direction, ConversationUnsupportedMediaType, "Direction is required")
