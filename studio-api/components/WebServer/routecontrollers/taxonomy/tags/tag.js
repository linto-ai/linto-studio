...
async function createTag(req, res, next) {
  try {
    ...
    const tag = await model.tags.getTagByCategoryAndName(req.body.categoryId, req.body.name)
...
async function updateTag(req, res, next) {
  try {
    ...
    const tag_search = await model.tags.getTagByCategoryAndName(req.body.categoryId, req.body.name)
    if (tag_search.length === 1 && tag[0]._id !== tag_search[0]._id) throw new TagConflict()
