...
async function updateMetadata(req, res, next) {
  try {
    ...
    if (result.result.nModified === 1) res.status(200).send({ message: 'Metadata updated' })
...
async function deleteMetadata(req, res, next) {
  try {
    ...
    if (result.deletedCount !== 1) throw new MetadataError('Error during the deletion of the metadata')
