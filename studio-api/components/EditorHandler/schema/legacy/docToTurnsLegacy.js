...
const { schema } = require("./serverSchemaLegacy")

/**
 * LEGACY — generation-1 read path (mark-based documents).
 *
 * Extract turns from a generation-1 Y.Doc, where word identity lives in the
 * inline `word` mark on each text node. Used ONLY during migration: read an
 * old editor state once, flush its content, then reseed as plain text.
 * Timestamps are NOT in the doc; words here carry only { wid, word }.
 *
 ...
 */
function docToTurnsLegacy(ydoc, field = "default") {
...
module.exports = { docToTurnsLegacy }
