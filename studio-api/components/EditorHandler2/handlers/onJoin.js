const debug = require("debug")("linto:components:EditorHandler2:onJoin")
...
async function onJoin({ socket }, conversationId, ack) {
  ...
  try {
    ...
    // users list and version arrive with their own iterations.
    reply({ ok: true, locks: locks.map(wireLock), users: [], version: 0 })
  } catch (err) {
