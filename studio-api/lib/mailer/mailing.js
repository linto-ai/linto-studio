...
class Mailing {
    ...
    async userMailNotificationProfile(user, Type) {
        try {
            ...
            if (user.emailNotifications === undefined) {
                let userSettings = await model.users.getByIdFilter(user.id, { emailNotifications: 1 })
                user = { ...user, emailNotifications: userSettings[0].emailNotifications }
            }
    ...
    */
    ...
    async organizationAccountCreate(email, origin, magicId, sharedByEmail, orgaName, organizationId) {
        if (typeof origin === 'object') origin = this.exctractRequestOrigin(origin)
    ...
    */
    ...
    async conversationShared(user, origin, sharedByEmail, conversationId) {
        if (!await this.userMailNotificationProfile(user, TYPE.SHARE_CONVERSATION)) return true

        if (typeof origin === 'object') origin = this.exctractRequestOrigin(origin)

        return await this.sendMail(TYPE.SHARE_CONVERSATION, { email: user.email, origin }, { origin, sharedByEmail, conversationId })
    }

    async conversationRightUpdate(user, origin, sharedByEmail, conversationId) {
        if (!await this.userMailNotificationProfile(user, TYPE.SHARE_CONVERSATION_RIGHT_UPDATE)) return true

        if (typeof origin === 'object') origin = this.exctractRequestOrigin(origin)

        return await this.sendMail(TYPE.SHARE_CONVERSATION_RIGHT_UPDATE, { email: user.email, origin }, { origin, sharedByEmail, conversationId })
    }

    async multipleConversationRight(user, origin, sharedByEmail, conversationsList) {
        if (!await this.userMailNotificationProfile(user, TYPE.SHARE_CONVERSATION_RIGHT_UPDATE)) return true

        if (typeof origin === 'object') origin = this.exctractRequestOrigin(origin)

        return await this.sendMail(TYPE.SHARE_MULTIPLE_CONVERSATION_RIGHT, { email: user.email, origin }, { origin, sharedByEmail, conversationsList })
    }

    async conversationUnshare(user, origin, name) {
        if (!await this.userMailNotificationProfile(user, TYPE.UNSHARE_CONVERSATION)) return true

        if (typeof origin === 'object') origin = this.exctractRequestOrigin(origin)

        return await this.sendMail(TYPE.UNSHARE_CONVERSATION, { email: user.email, origin }, { name })
    }

    async organizationInvite(user, origin, sharedByEmail, orgaName, organizationId) {
        if (!await this.userMailNotificationProfile(user, TYPE.INVITE_ORGANIZATION)) return true

        if (typeof origin === 'object') origin = this.exctractRequestOrigin(origin)

        return await this.sendMail(TYPE.INVITE_ORGANIZATION, { email: user.email, origin }, { origin, sharedByEmail, orgaName, organizationId })
    }

    async organizationRightUpdate(user, origin, orgaName) {
        if (!await this.userMailNotificationProfile(user, TYPE.UPDATE_ORGANIZATION_RIGHT)) return true

        if (typeof origin === 'object') origin = this.exctractRequestOrigin(origin)

        return await this.sendMail(TYPE.UPDATE_ORGANIZATION_RIGHT, { email: user.email, origin }, { origin, orgaName })
    }

    async organizationDelete(user, origin, orgaName) {
        if (!await this.userMailNotificationProfile(user, TYPE.DELETE_ORGANIZATION_RIGHT)) return true

        if (typeof origin === 'object') origin = this.exctractRequestOrigin(origin)

        return await this.sendMail(TYPE.DELETE_ORGANIZATION_RIGHT, { email: user.email, origin }, { origin, orgaName })
    }
