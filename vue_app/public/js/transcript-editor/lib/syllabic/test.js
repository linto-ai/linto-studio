...
async function test() {
    ...
    const dict = require(`./dict/${syllabic.language}.json`)
    const erroredWords = []
    const perfectWords = []
    ...
    const deviationsCalc = Math.sqrt(deviations.reduce((a, b) => a + b * b, 0) / deviations.length)
    ...
    console.log(`Average error for errored words: ${deviations.reduce((a, b) => a + b, 0) / deviations.length}`)
        //global accuracy
    const allWords = erroredWords.concat(perfectWords)
    const globalDeviations = allWords.map(word => Math.abs(word[1] - word[2]))
    const globalDeviationsCalc = Math.sqrt(globalDeviations.reduce((a, b) => a + b * b, 0) / globalDeviations.length)
    console.log(`Standard deviation for a given word: ${globalDeviationsCalc}`)
    console.log(`Average error for a given word: ${globalDeviations.reduce((a, b) => a + b, 0) / globalDeviations.length}`)
        //Top errored words
    console.log("Top errored words", "\n", "-------------------")
        //print errored words with errors count
    let topErroredWords
    for (let i = 1; i < 10; i++) {
        topErroredWords = erroredWords.filter(word => Math.abs(word[2] - word[1]) == i)
