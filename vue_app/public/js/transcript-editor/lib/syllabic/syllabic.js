...
class Syllabic {
    ...
    count(term) {
        ...
        if (wordInError) {
            ...
            return syllabation.call(this, term).length
        }
    }

    syllabify(term) {
        return syllabation.call(this, term)
    }
}
...
function syllabation(term) {
    ...
    const result = []
    const re = new RegExp(`(${this.rules.join('|')})`, 'g');
