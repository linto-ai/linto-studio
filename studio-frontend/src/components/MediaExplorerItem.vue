<template>
    <div class="media-explorer-item" @mouseenter="hover" @mouseleave="leave" @click="select"
        :class="{ 'media-explorer-item--hover': isHover, 'media-explorer-item--selected': isSelected, 'media-explorer-item--favorite': isFavorite }">
        <div class="media-explorer-item__inline">
            <div class="media-explorer-item__inline__meta actions">
                <span class="media-explorer-item__inline__favorite" :class="{ 'active': isFavorite }"
                    @click.stop="toggleFavorite">
                    <ph-star animation="pulse" :weight="isFavorite ? 'fill' : 'regular'" />
                </span>
                <!-- Media type icon: when not hovered-->
                <div class="media-explorer-item__inline__selection" :class="{ 'selected': isSelected }">
                    <!-- Selection checkbox: when hovered -->
                    <div class="media-explorer-item__inline__checkbox" :class="{ 'active': isSelected }">
                        <input type="checkbox" v-model="isSelected" />
                    </div>
                </div>
            </div>
            <div class="media-explorer-item__inline__meta flex1 gap-small">
                <UserProfilePicture :user="convOwner" class="media-explorer-item__inline__infos__user" />
                <a href="#">{{ title }}</a>
                <div class="media-explorer-item__inline__infos">
                    <span v-if="duration" class="media-explorer-item__inline__infos__duration">
        ...
        convOwner() {
            ...
            if (owner) {
                return {
                    ...
                    img: owner.img ? process.env.VUE_APP_PUBLIC_MEDIA + "/" + owner.img : null,
                }
            } else {
                return {
