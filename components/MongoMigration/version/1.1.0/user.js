...
module.exports = {
  up: async (db) => {
    ...
    db.collection(collections_name).updateMany({}, { $unset: migration_update_remove })

    // Get user mail to add it in verifiedEmail
    const users = await db.collection(collections_name).find({}).toArray()
    users.forEach(async (user) => {
      if (user.email) {
        await db.collection(collections_name).updateOne({ _id: user._id }, { $push: { verifiedEmail: user.email } })
      }
    })
  },
  down: async (db) => {
    ...
    db.collection(collections_name).updateMany({}, { $set: migration_update_remove })
