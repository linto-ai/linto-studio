...
async function identifySpeaker(req, res, next) { //WIP
    try {
        ...
        // Check if response.result and response.results['ok'] "exists" in the response, and its value
        if (!!response.result && !!response.result['ok'] && response.result['ok'] === 1) {
            // RESPONSE 
            res.json({
                status: "200",
                msg: "success!"
        ...
        res.json({
            status: 'error',
            msg: 'error on identifying speaker'
        })
    }
}
