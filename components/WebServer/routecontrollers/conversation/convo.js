...
async function renumberWords(req, res, next){ //WIP
    try{
        ...
        let response = await convoModel.getAllWords(payload)
            console.log(response[0].text[0].words) //gives back a list of words
        if (response.length > 0){
...
async function updateWordText(req, res, next){ //WIP
    try {
        ...
        //delete, reposition 
            ...
            //retrieve start time, end time and positions
            //calculate new times
...
async function deleteWordText(req, res, next){ //WIP
    try{
        const payload = req.body
