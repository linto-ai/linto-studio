const debug = require('debug')('linto:components:WebServer:controller:file:tranform')
...
async function transformAudio(filePath, transformedFilePath) {
  let streamProcess = spawn("ffmpeg", ['-i', `${filePath}`, ...ARG_TRANSFORM, transformedFilePath], { detached: true })
...
async function mergeAudio(files, audioPath) {
  ...
  let streamProcess = spawn("ffmpeg", [...fileList, '-filter_complex', filter_complex, ...ARG_TRANSFORM, audioPath],
    { detached: true })
  await handleStreamProcess(streamProcess)
}
