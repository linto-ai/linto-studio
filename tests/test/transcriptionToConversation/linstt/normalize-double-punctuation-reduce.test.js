const debug = require('debug')('linto:tests:linstt:normalize-double-punctuation-reduce')
  ...
  it('segment with no filter and rules of double punctuation', () => {
    ...
    normalizeTranscription.segments.map((segment, index_seg) => {
      ...
      segment.words.map((words, index_word) => {
        // debug('words', words)
        // debug('conversation.text[index_seg].words[index_word]', conversation.text[index_seg].words[index_word])
